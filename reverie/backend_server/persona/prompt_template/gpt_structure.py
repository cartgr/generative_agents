...
def GPT_request(prompt, gpt_parameter):
    """
    Given a prompt and a dictionary of GPT parameters, make a request to the OpenAI
    server and return the response.

    ARGS:
      prompt: a str prompt.
      gpt_parameter: a python dictionary with the keys indicating the names of
                     the parameters and the values indicating the parameter
                     values.
    ...
    try:
        ...
        response = openai.ChatCompletion.create(
            model=gpt_parameter["engine"],  # e.g., "gpt-4" or "gpt-40-mini"
            messages=[
